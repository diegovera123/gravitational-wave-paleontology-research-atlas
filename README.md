...
> **Current navigation (September 2026): exactly two sections — Home and Knowledge Graph.** Home is a calm introduction with an optional concept-based starting-point diagnostic and a small, context-sensitive Otto helper. Knowledge Graph is a full-width, progressively expanding 3D research constellation. Selecting a concept opens its learning unit **inside the graph**, including actual necessary/useful prerequisites, objectives, original exercise ideas, public resource links and related research questions. The three fully authored pilot lessons and their five-question adaptive practice run **inside Knowledge Graph** rather than a separate Learn & Practice tab. Other concepts offer only existing single-item conceptual checks or explicitly ungraded exercise prompts. Research questions are in a collapsible section of the graph and linked to relevant concept units. The previous Research, Starting Point, Learn & Practice and Resources top-level tabs and right-hand structured-map sidebar are not user-visible; legacy DOM remains hidden only for older rendering/deep-link functionality. Otto is a small scripted contextual guide, not an AI chatbot. See [two-section architecture](docs/two-section-research-atlas.md). Historical sections below describe earlier designs.
