...
> **Current navigation (September 2026): Home · Knowledge Graph · Practice.** Home has the optional concept diagnostic and Otto. Knowledge Graph contains the progressive 3D hierarchy with **explicit learning objectives** for each scientific concept, its real prerequisite links, authored pilot lessons, a single contextual Research questions section, original curated resource links and an optional live search of up to 1,000 OpenAlex scholarly metadata hits per concept (paged, not pre-curated or verified). Every one of the 133 concepts has **100 objective-anchored, progressively staged self-check research prompts** generated from ten pedagogical tiers and ten task forms (13,300 prompts across the curriculum); these are **not 13,300 independently authored numerical problems, automatically graded assessments or verified worked solutions**. The same 100-prompt progression is available inside each related Practice track. Practice retains **168 independently authored multiple-choice questions across 15 tracks** and **24 worked ungraded research cases**. Research search results may contain unrelated or non-peer-reviewed works and require independent screening. See [practice workspace notes](docs/practice-workspace.md). Historical project descriptions below reflect earlier versions.
