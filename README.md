...
> **Research question deep dives (September 2026):** Pick any of six independent research questions, then optionally select one of three concept-specific investigations for that topic (**18 explicitly authored inquiries** in total). Each focused inquiry includes a conditional, testable expectation, a concrete toy-model method, scientific limitations and links to the relevant Knowledge Graph concept and toy lab. An explicit action can copy the selected question into the existing browser-local research notebook **only if its question field is empty**; existing research notes are never silently overwritten. No parent or focused question is auto-selected on first visit or reload. The six-question chooser, seven educational models and thirty worked research problems remain available within Practice; these are educational investigations, not actual calibrated astrophysical results.

> **Research question chooser (September 2026):** The Practice research area now shows six equally selectable scientific questions instead of a numbered field-to-research sequence. No question, lab, notes or exercises open until a user selects a card; clicking an open card closes it. No suggested earlier stage or prerequisite gating appears. Previously saved exploration flags and local notes are preserved, but the last open question is not automatically reopened. Detailed research content and simulations remain under the selected question, with authored Practice exercises unchanged.

> **Current scientific-content expansion (September 2026):** 37 individually authored, scenario-first concept explanations (up from 25), with four substantive mechanism steps, an original worked example and model limits. The six-stage research pathway now has **30 authored worked problems (five per stage)**, presented in expandable cards in Practice, plus a seventh interactive educational model of two stellar-birth cohorts and fixed formation-to-merger delays. Seven additional topic-specific scientific schematics cover the IMF, core collapse, quadrupole radiation, proper time, metallicity mixtures, dynamical assembly and time-series sampling. These are illustrations and intentionally simplified toy models, not measured detector data, validated research computations or a substitute for astrophysicist review. The Knowledge Graph remains one continuous concept reader; all problem-solving stays in Practice.

> **Integrated research-learning pilot (September 2026).** The existing **three** top-level destinations remain Home, Knowledge Graph and Practice. Inside Practice, a six-stage research pathway follows one binary from orbital foundations through massive-star evolution, supernova survival, gravitational waves, synthetic population modeling and selection-aware inference. Each stage maps to real graph concepts, a corresponding authored Practice track, three individually authored worked scientific reasoning tasks, and a sequence of existing public source links with clearly limited reading guidance. Six **interactive, parameter-responsive educational toy models** now cover instantaneous natal kicks, leading-order quasi-circular chirps, a common-envelope energy ledger, Roche-lobe geometry, seeded synthetic binary outcomes and a two-class detector-selection example, with explanatory SVGs and explicit physical limits. Synthetic records can be exported to CSV with their seed and toy provenance. An independent browser-local research notebook records hypotheses, controlled methods, evidence, uncertainties and next steps, with Markdown export. Existing authored assessment responses are summarized by knowledge component as response counts and due-review cues, **not learner ability/mastery estimates**. The 3D Knowledge Graph also distinguishes a small number of authored, typed prerequisite/causal/application relationships from containment edges. The pathway contains optional prompts that can complement research-group onboarding but is **not official RAKIURA curriculum** and does not execute COMPAS, analyze private files, fetch experimental data, or certify research readiness. This is an integrated, test-covered first implementation of the requested eight directions, **not** completion of every deep lesson, validated problem bank, paper curation, realistic astrophysical simulator or official onboarding workflow. See [integrated research pilot notes](docs/integrated-research-pilot.md) for exact scope and testing limits.

> **Current reading experience (September 2026):** The Knowledge Graph preserves three primary navigation destinations (Home, Knowledge Graph, Practice) and presents each concept as one continuous, expandable learning unit: objectives first, mechanism-led explanation and worked illustration next, connections and research resources last. **Scientific diagrams are now specific to the physical concept**: schematic gravitational-wave polarization and strain chirps, a natal-kick vector addition, Roche-lobe overflow, detector selection, population-synthesis branching, and cosmic delay time. These figures are original educational illustrations with accessible textual descriptions, **not real detector data or simulation output**. A narrow reading-position indicator and back-to-top action support longer lessons without reintroducing content tabs; explanatory figures remain legible on small screens by scrolling within the figure. Twenty-five concepts now have individually authored, concrete multi-step explanations with worked examples; the remaining concepts are explicitly labeled as shorter orientations awaiting scientific content. All guided exercises stay in the dedicated Practice section. See [practice workspace notes](docs/practice-workspace.md). Older project descriptions below document historical iterations.


> **Current 3D Explorer (September 2026):** The visible Knowledge Map is now a **single, full-width interactive 3D constellation**, not the former structured 2D map, map-mode toggle, or large concept-details sidebar. Select one of five major gravitational-wave paleontology clusters to reveal its existing scientific regions, then its curated topics, and finally individual concepts. Click a concept for a small preview and choose **Explore this concept** to open the existing focused explanation, real prerequisites and available drills. The Back and All Clusters buttons move up the hierarchy; accessible labeled buttons below the canvas provide equivalent keyboard navigation and a usable non-WebGL fallback. The homepage links directly to this 3D experience rather than displaying the old flat ten-region graph. 3D initializes only when opened. Legacy explorer DOM remains hidden exclusively to support older internal deep links/progress routines; it is not an alternate visible interface. See [3D Explorer architecture](docs/immersive-constellation.md). Earlier README sections about a default structured-map dashboard describe retired interface iterations.
