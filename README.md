...
> **Current navigation (September 2026): Home · Knowledge Graph · Practice.** Home offers the optional concept diagnostic and Otto. Knowledge Graph contains the interactive 3D hierarchy, concept explanations, sources and full original pilot lessons. Practice is a separate third tab with the three authored five-question adaptive pilot sets, review-due objectives and formative feedback. Opening practice from a graph concept selects that concept's real set; other graph concepts retain their ungraded prompts and single-question checks, without fabricated exercise banks. Saved adaptive history remains browser-local. See [practice workspace notes](docs/practice-workspace.md). Historical two-section details below describe the previous implementation.
