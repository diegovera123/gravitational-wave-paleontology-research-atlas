...
**Source handling:** these three pilot explanations were independently authored, then linked to public materials for scientific checking. Public links are not a grant to copy source text, figures or datasets. In particular, the external OpenStax textbook's current reuse terms require special care for noncommercial/adapted uses and expressly restrict AI ingestion; the Atlas only links to it rather than reproducing its text. COMPAS data examples use invented records, not a downloaded astrophysical dataset. No unpublished VIMES manuscript, private lab material, or full copyrighted paper is republished.
