...
> **Current interface (September 2026): Home · Knowledge Graph · Practice.** Home retains the optional diagnostic and Otto. The Knowledge Graph is an immersive 3D constellation: selecting a concept opens a **compact, expandable four-tab overlay**, rather than appending an enormous detail page below the graph. Its tabs are **Intuition & visuals**, **Learning objectives**, **Connections** and **Research resources**. Intuitive explanations, limitations and clearly labeled conceptual schematics are provided for key astrophysics topics; all other concepts have objective-anchored orientation and a labeled general schematic. Curated references and optional on-demand scholarly metadata discovery remain under Resources. **Written prompts, quick quizzes and longer practice all live in the separate Practice section**, reachable directly from every graph concept. Practice retains 15 graded pilot/extended tracks with 168 independently authored multiple-choice questions and 24 worked ungraded cases, plus a self-checked 100-step guided progression for every one of the 133 mapped concepts (no scored bank is fabricated for concepts without one). The prompt progression is a learning scaffold, not 13,300 independently authored or graded numerical problems. The graph contains one Research questions section. See [practice workspace notes](docs/practice-workspace.md). Historical version descriptions below are retained as project history.
