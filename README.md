...
> **Current navigation (September 2026): Home · Knowledge Graph · Practice.** Home offers the optional concept diagnostic and Otto. Knowledge Graph contains the interactive 3D hierarchy, concept explanations, sources and three fully authored original pilot lessons. Practice has **15 assessed tracks**: the existing three pilots and **12 new research-area tracks spanning orbits, stellar evolution, binary interactions, compact remnants, gravitational waves, detection, simulations, selection effects, cosmic merger histories, inference and reproducibility**. The expanded bank has **168 independently authored multiple-choice questions** linked to explicit knowledge components, plus **24 worked self-checked applied research cases**. Each new track offers a 12-question cross-component assessment and focused three-question component checks. Quiz feedback and due-review history remain browser-local; self-written case responses are not stored or graded. The 12 new tracks are **practice resources, not fictitious full lessons**, and link to their related real 3D graph concepts. See [practice workspace notes](docs/practice-workspace.md). Historical earlier navigation text below is retained as project history.
