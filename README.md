...
> **Current concept layout (September 2026):** Clicking a concept opens **one continuous, expandable reading unit**, not four tabs and not an extended page below the 3D graph. Its reading order is **Learning objectives → concrete, step-by-step explanation → worked illustrative example → clearly labeled conceptual diagram and model limits → prerequisite/downstream connections → public resources and related-paper discovery at the bottom**. The first **12 key astrophysics concepts** now have explicitly authored substantial scenario-first lessons with mechanisms, stepwise reasoning, illustrative calculations and limits (in `knowledge-graph/deep-explanations.json`). Remaining concepts retain clearly labeled shorter orientations pending individually authored content; they do not pretend to have long textbook-level explanations yet. The overlay remains scrollable, compact or expandable, with a single link to the separate Practice tab. Practice retains 15 authored scored tracks (168 questions), 24 worked self-check cases and the 100-step self-checked prompt progression for all 133 concepts. The Knowledge Graph has one Research questions section. See [practice workspace notes](docs/practice-workspace.md). Historical project descriptions below are retained as implementation history.
