...
assert.equal(journey.inquiries.length,18,"Three independently authored research investigations under each of the six questions");
for(const s of journey.stages)assert.equal(journey.inquiries.filter(q=>q.stageId===s.id).length,3,"Every question offers three independently selectable inquiries");
...
assert.doesNotMatch(host.innerHTML,/class="research-inquiries"|data-inquiry=/,"No subquestion is presented as a required starting point");
...
assert.equal((host.innerHTML.match(/data-inquiry="/g)||[]).length,3,"Selected question exposes three focused optional inquiries");
assert.doesNotMatch(host.innerHTML,/id="research-inquiry-detail"/,"No focused inquiry is selected by default");
handlers.click({target:{closest:()=>({dataset:{inquiry:"inference-bias"}})}});
assert.equal(instance.snapshot().selectedInquiry,"inference-bias","Any specific scientific question can be selected independently");
assert.match(host.innerHTML,/id="research-inquiry-detail"/,"Selecting an inquiry reveals its method, hypothesis and limitations");
assert.match(host.innerHTML,/Read related concept/);
handlers.click({target:{closest:()=>({dataset:{action:"use-inquiry"}})}});
assert.equal(instance.snapshot().notes.inference.question,journey.inquiries.find(q=>q.id==="inference-bias").question,"Notebook handoff uses the selected question");
handlers.click({target:{closest:()=>({dataset:{inquiry:"inference-rate"}})}});
handlers.click({target:{closest:()=>({dataset:{action:"use-inquiry"}})}});
assert.equal(instance.snapshot().notes.inference.question,journey.inquiries.find(q=>q.id==="inference-bias").question,"A second inquiry never silently overwrites existing research notes");
handlers.click({target:{closest:()=>({dataset:{action:"inquiry-lab"}})}});
assert.equal(instance.snapshot().lab,"selection","The selected inquiry opens its own specified toy lab");
handlers.click({target:{closest:()=>({dataset:{inquiry:"inference-rate"}})}});
assert.equal(instance.snapshot().selectedInquiry,null,"Clicking selected inquiry again collapses its details");
...
assert.equal(instance.snapshot().selectedInquiry,null,"Closing a parent question clears its selected focused inquiry");
...
assert.equal(reopened.snapshot().selectedInquiry,null,"A visit never auto-selects a specific inquiry");
